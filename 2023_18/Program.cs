...
void part2()
{
    ...
    long pathLength = 0;
    foreach ((var line, int li) in lines.Select((str, i) => (str, i)))
    {
        long num = Int32.Parse(line.Colour[0..^1], System.Globalization.NumberStyles.HexNumber);
        var dir = hexToDir(line.Colour);
        var prevDir = hexToDir(lines[(li + lines.Count - 1) % lines.Count].Colour);
        var nextDir = hexToDir(lines[(li + 1) % lines.Count].Colour);

        current = current + (num + 1 - (turnR(prevDir, dir) ? 0 : 1) - (turnR(dir, nextDir) ? 0 : 1)) * move(dir);
        pathLength += num;
    ...
    {
        var ys = (long)vertices[v + 1].Real + (long)vertices[v].Real;
        var xs = (long)vertices[v + 1].Imaginary - (long)vertices[v].Imaginary;
        part2 += xs * ys;
    }
...
bool turnR(char from, char to) => (from, to) switch
{
    ('R', 'D') => true,
    ('R', 'U') => false,
    ('D', 'L') => true,
...
char hexToDir(string hex) => hex.Last() switch
    {
        '0' => 'R',
        '1' => 'D',
        '2' => 'L',
